...
#create an exmpty list to append variant scores 
variant_scores = []

#iterate through files and append variant score to empty list 
for ex in exomiser_files:
	df = pd.read_csv(ex, sep='\t')
	for index, row in df.iterrows():
		variant_scores.append(row[30])

#plot histogram of distribution ot variant scores 
plt.hist(variant_scores, range=(0, 1), color = 'orangered', edgecolor = 'black')
plt.xlabel('Variant Score', labelpad=10)
plt.ylabel('Frequency', labelpad=10)
plt.title('Distribution of Variant Scores for all Variants', pad=10)
plt.show()
